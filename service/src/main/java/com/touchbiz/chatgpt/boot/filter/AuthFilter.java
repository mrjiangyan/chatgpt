...
public class AuthFilter implements WebFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (StringUtils.isEmpty(token)) {
            ...
            if (StringUtils.isEmpty(token)) {
                 return outputMessage(exchange.getResponse(), IResultMsg.APIEnum.NOT_LOGIN_ERROR, "token is not exist");
            }
        }
        ...
        try {
            ...
            log.error("", ex);
            return outputMessage(exchange.getResponse(), IResultMsg.APIEnum.NOT_LOGIN_ERROR, ex.getMessage());
        }
        return chain.filter(exchange);
    }

    private Mono<Void> outputMessage(ServerHttpResponse response, IResultMsg resultMsg, String source) {
        ...
        return response.writeAndFlushWith(Flux.just(ByteBufFlux.just(response.bufferFactory().wrap(JsonUtils.toJson(ApiResult.getCustomResponse(resultMsg)).getBytes()))));
    }
